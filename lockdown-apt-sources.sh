...
# same name. unattended-upgrades is likewise restricted to Ubuntu's own hosts, see Unattended upgrades below.
#
#
# Usage
# -----
...
# Unattended upgrades
# -------------------
# unattended-upgrades picks the repositories it installs from by matching the Origin and Suite of each repository's
# own Release file, so a third-party repository declaring "Origin: Ubuntu" and "Suite: resolute-security" would have
# its upgrades installed unattended. APT refuses such a change of an already added repository until it is accepted
# explicitly, but interactive apt update asks whether to accept it, and a repository can declare it from the start.
# The lockdown pins still limit it to the packages allowed from that repository.
#
# The script therefore writes /etc/apt/apt.conf.d/51unattended-upgrades-lockdown, which clears the Allowed-Origins
# of 50unattended-upgrades and restates each of them from UU_ORIGINS as an Origins-Pattern that also names the
# site it has to come from. 50unattended-upgrades belongs to the unattended-upgrades package, so an upgrade of that
# package can change which origins it allows. The dry-run and --apply report when that set no longer matches
# UU_ORIGINS, and when any origin is still allowed without a site. Update UU_ORIGINS to match and --apply again.
#
#
...
# Sorts after 50unattended-upgrades, whose Allowed-Origins it clears.
UU_NAME=51unattended-upgrades-lockdown
UU_DEST=/etc/apt/apt.conf.d/$UU_NAME
# The origins unattended-upgrades may install from, written as 50unattended-upgrades writes them in Allowed-Origins,
# each followed by the site it has to come from. check_uu reports when 50unattended-upgrades allows a different set.
UU_ORIGINS='
${distro_id}:${distro_codename}                        *.ubuntu.com
${distro_id}:${distro_codename}-security               *.ubuntu.com
${distro_id}ESMApps:${distro_codename}-apps-security   esm.ubuntu.com
${distro_id}ESM:${distro_codename}-infra-security      esm.ubuntu.com
'
...
# Emit the unattended-upgrades configuration on stdout.
generate_uu() {
  cat <<'UU_EOF'
// Generated by lockdown-apt-sources.sh. Rewritten whole on every run, so edits made here are lost.
//
// unattended-upgrades picks the repositories it installs from by the Origin and Suite each declares in its own
// Release file. A third-party repository can declare "Origin: Ubuntu" and "Suite: resolute-security" and have its
// upgrades installed unattended: one that declares it from the start, or one whose change to it was accepted when
// apt update asked. The lockdown pins still apply, which limits that to the packages allowed from it. The allowed origins of
// 50unattended-upgrades are cleared here and restated with the site each has to come from, the host of the source,
// which comes from our own configuration and which a repository cannot choose.
#clear Unattended-Upgrade::Allowed-Origins;
Unattended-Upgrade::Origins-Pattern {
UU_EOF
  local entry site
  while read -r entry site; do
    [ -n "$entry" ] || continue
    printf '\t"origin=%s,archive=%s,site=%s";\n' "${entry%%:*}" "${entry#*:}" "$site"
  done <<< "$UU_ORIGINS"
  echo "};"
}

# Check the unattended-upgrades configuration with the given file in place of the installed one. The origins allowed
# without it, as 50unattended-upgrades and any other file ship them, have to be the ones restated in UU_ORIGINS, and
# with it no origin may be left allowed without a site. Returns non-zero and names each difference when not.
check_uu() {
  python3 - "$1" "$UU_NAME" "$UU_ORIGINS" <<'PY_EOF'
import apt_pkg, glob, os, re, sys

candidate, name, table = sys.argv[1], sys.argv[2], sys.argv[3]
restated = [line.split()[0] for line in table.splitlines() if line.strip()]

# APT reads a file from apt.conf.d only when its name consists of these characters, and silently skips backups and
# leftovers of package upgrades.
VALID = re.compile(r"[A-Za-z0-9_.-]+")
SKIP = re.compile(r"(~|\.disabled|\.bak|\.dpkg-[a-z]+|\.ucf-[a-z]+|\.save|\.orig|\.distUpgrade)$")

def load(with_candidate):
    cnf = apt_pkg.Configuration()
    names = {os.path.basename(p) for p in glob.glob("/etc/apt/apt.conf.d/*")} | {name}
    for n in sorted(names):
        if not VALID.fullmatch(n) or SKIP.search(n):
            continue
        if n == name:
            if with_candidate:
                apt_pkg.read_config_file(cnf, candidate)
        else:
            apt_pkg.read_config_file(cnf, "/etc/apt/apt.conf.d/" + n)
    if os.path.exists("/etc/apt/apt.conf"):
        apt_pkg.read_config_file(cnf, "/etc/apt/apt.conf")
    return cnf

shipped = load(False).value_list("Unattended-Upgrade::Allowed-Origins")
locked = load(True)
problems = []
problems += ["allowed as shipped but not restated in UU_ORIGINS, so no longer allowed: " + o
             for o in shipped if o not in restated]
problems += ["restated in UU_ORIGINS but no longer allowed as shipped: " + o
             for o in restated if o not in shipped]
# A file sorting after this one can add to Allowed-Origins again, which the clear here does not reach.
problems += ["still allowed without a site: " + o for o in locked.value_list("Unattended-Upgrade::Allowed-Origins")]
problems += ["pattern without a site: " + p for p in locked.value_list("Unattended-Upgrade::Origins-Pattern")
             if not any(t.strip().startswith("site=") for t in p.split(","))]

for p in problems:
    print("   " + p)
print("UNATTENDED-UPGRADES FAILED: %d problem(s)" % len(problems) if problems
      else "UNATTENDED-UPGRADES OK: allowed origins match the shipped ones, all restricted to a site")
...
case "$MODE" in
  ...
  dry-run)
    tmp=$(mktemp -d); uu=$(mktemp); trap 'rm -rf "$tmp" "$uu"' EXIT
    build_candidate "$tmp"
    generate_uu > "$uu"
    ...
    verify "$tmp"
    echo
    echo "=== would write $UU_DEST ==="
    if [ -e "$UU_DEST" ]; then
      diff -u "$UU_DEST" "$uu" && echo "   (no change)"
    else
      echo "   new file"
    fi
    echo
    echo "=== unattended-upgrades against the candidate configuration ==="
    check_uu "$uu" || true
    echo
  ...
  apply)
    ...
    if [ "$changed" = false ]; then
      ...
      # The shipped unattended-upgrades configuration can change on its own with a package upgrade.
      if ! report=$(check_uu "$UU_DEST"); then
        echo
        echo "$report"
      fi
      exit 0
    ...
    [ -e "$UU_DEST" ] && cp -a "$UU_DEST" "$BACKUP/"
    echo "backed up /etc/apt/preferences.d and $UU_DEST to $BACKUP/"
